...
import com.example.aapplication.mp.io.UserInputModule;
import com.example.aapplication.mp.view.ColorView;

import static com.example.aapplication.mp.view.ColorView.SHAPE_CIRCLE;

class MaterialColorPreference extends AbsMaterialPreference<Integer> implements
    ...
    protected void onCollectAttributes(AttributeSet attrs) {
        TypedArray ta = getContext().obtainStyledAttributes(attrs,R.styleable.MaterialColorPreference);
        try {
            config = new ColorIndicatorConfig();
            config.setBorderColor(ta.getColor(R.styleable.MaterialColorPreference_mp_border_color, Color.BLACK));
            config.setBorderWidth(ta.getDimensionPixelSize(R.styleable.MaterialColorPreference_mp_border_width, 1));
            config.setShape(ta.getInt(R.styleable.MaterialColorPreference_mp_indicator_shape, SHAPE_CIRCLE));

            initialColor = ta.getColor(R.styleable.MaterialColorPreference_mp_initial_color, Color.WHITE);
        } finally {
            ta.recycle();
        }
    }
