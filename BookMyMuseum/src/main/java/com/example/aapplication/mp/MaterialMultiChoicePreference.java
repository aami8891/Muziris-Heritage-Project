...
public class MaterialMultiChoicePreference extends AbsMaterialListPreference<Set<String>> {
    ...
    protected void onCollectAttributes(AttributeSet attrs) {
        ...
        TypedArray ta = getContext().obtainStyledAttributes(attrs, R.styleable.MaterialMultiChoicePreference);
        try {
            if (ta.hasValue(R.styleable.MaterialMultiChoicePreference_mp_default_selected)) {
                CharSequence[] defSelected = ta.getTextArray(R.styleable.MaterialMultiChoicePreference_mp_default_selected);
